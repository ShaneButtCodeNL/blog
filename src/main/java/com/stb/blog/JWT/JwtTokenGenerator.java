...
import com.stb.blog.actions.Actions;
...
public class JwtTokenGenerator {
    private final int ONE_MIN = 1000*60;
    private final int FIVE_MINS = 1000*60*5;
    ...
    public TokenReturn createRefreshToken(User user){
        Date iss = Actions.getDateInGMT(new Date());
        Date exp = Actions.getDateInGMT(new Date(new Date().getTime() + ONE_DAY));
        return new TokenReturn(Jwts.builder()
                .setIssuer("SERVER")
                .setSubject(user.getUsername())
                .setExpiration(exp)
